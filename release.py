...
def require_clean_git_tree() -> None:
    ...
    if result.stdout.strip():
        print("Git working tree must be clean before creating a release.", file=sys.stderr)
...
def replace_version(path: Path, pattern: str, new_version: str) -> None:
    ...
    if replacements != 1:
        raise SystemExit(f"Expected to update exactly one version in {path.relative_to(ROOT)}.")
...
def create_commit_and_tag(version: str, *, dry_run: bool) -> None:
    ...
    if dry_run:
        print(f"[dry-run] Would run: git add {MANIFEST_PATH.relative_to(ROOT)} {PYPROJECT_PATH.relative_to(ROOT)}")
...
def create_github_release(tag_name: str, *, github_release: bool, dry_run: bool) -> None:
    if not github_release:
        return

    command = ["gh", "release", "create", tag_name, "--generate-notes", "--title", tag_name]
    if dry_run:
        print(f"[dry-run] Would run: {' '.join(command)}")
        return
    run_command(command)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prepare a new release by updating versions, tagging, and optionally pushing."
    )
...
def main() -> int:
    ...
    if args.github_release and not args.push:
        print("--github-release requires --push so the tag exists on GitHub.", file=sys.stderr)
        return 1
    ...
    create_github_release(tag_name, github_release=args.github_release, dry_run=args.dry_run)
    ...
    if args.dry_run:
        ...
        if args.push and not args.github_release:
            print(f"Next step: gh release create {tag_name} --generate-notes --title {tag_name}")
